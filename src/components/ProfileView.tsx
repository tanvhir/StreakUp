...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const [targetHours, setTargetHours] = useState(currentUser?.targetHoursPerDay || 8);
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-6 space-y-4 sm:space-y-6 pb-24 animate-fade-in">
          ...
          {userLogs.map((log) => (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center space-x-2">
                  <span className="font-extrabold text-slate-900 text-xs sm:text-sm">
                    {formatCycleDateLabel(log.cycleDate)}
                  </span>
                </div>
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-400 font-medium pt-1 border-t border-slate-200/50">
                <span>
                  Logged at {new Date(log.loggedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
